...
<div class="col-sm-12" id="card_attachment_{{ $attachment->attachment_uniqiueid }}">
    <div class="file-attachment">
        ...
        @else
        <div class="x-image">
            @if($attachment->attachment_extension == 'mp3' || $attachment->attachment_extension == 'wav')
            <audio controls>
                <source src="tasks/download-attachment/{{ $attachment->attachment_uniqiueid }}" type="audio/mpeg">
            </audio>
        ...
        <div class="x-details">
            <div>
                <span class="x-meta">{{ $attachment->first_name ?? runtimeUnkownUser() }}</span>
                [{{ runtimeDateAgo($attachment->attachment_created) }}]
            </div>
            <div class="x-name">
                <span title="{{ $attachment->attachment_filename }}">{{ str_limit($attachment->attachment_filename, 60) }}</span>
            </div>
            <div class="x-actions"><strong>
                    <!-- Action: download -->
                    <a href="tasks/download-attachment/{{ $attachment->attachment_uniqiueid }}" download>
                        {{ cleanLang(__('lang.download')) }} <span class="x-icons"><i class="ti-download"></i></span>
                    </a>
                </strong>
                <!-- Action: delete -->
                ...
                <span> |
                    <a href="javascript:void(0)" class="text-danger js-delete-ux-confirm confirm-action-danger"
                        ...
                        data-url="{{ urlResource('/tasks/delete-attachment/'.$attachment->attachment_uniqiueid) }}">
