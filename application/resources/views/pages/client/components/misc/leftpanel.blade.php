...
<div class="card">
    ...
    <div>
        <hr> </div>
    @if(config('system.settings_modules_invoices') == 'enabled' || config('system.settings_modules_payments') == 'enabled' 
    || config('system.settings_modules_tickets') == 'enabled')
    <div class="card-body p-t-0 p-b-0">
        <div>
            <table class="table no-border m-b-0">
                <tbody>
                    <!--invoices-->
                    @if(config('system.settings_modules_ivoices') == 'enabled')
                    <tr>
    ...
    @if(config('visibility.client_show_custom_fields') && $fields->count()>0)
    <!--CUSTOMER FIELDS-->
