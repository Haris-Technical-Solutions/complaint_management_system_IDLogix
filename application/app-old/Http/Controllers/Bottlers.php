...
use App\Http\Responses\Bottler\CreateResponse;
use App\Http\Responses\Bottler\EditResponse;
use App\Http\Responses\Bottler\IndexResponse;
use App\Http\Responses\Bottler\StoreResponse;
use App\Http\Responses\Bottler\UpdateResponse;
...
class Bottlers extends Controller {
    ...
    public function __construct(
        ...
        $this->middleware('bottlerMiddlewareIndex')->only([
            'index',
            'update',
            'store',
        ]);

        $this->middleware('bottlerMiddlewareCreate')->only([
            'create',
            'store',
        ]);

        $this->middleware('bottlerMiddlewareEdit')->only([
            'edit',
            'update',
            'destroy',
        ]);
    ...
    public function index() {
        ...
        //reponse payload
        $payload = [
            'page' => $this->pageSettings('bottler'),
            'users' => $users,
        ];
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        $page = [
            ...
            'submenu_team' => 'active',
            'sidepanel_id' => 'sidepanel-filter-bottler',
            'dynamic_search_url' => 'bottler/search?source=' . request('source') . '&action=search',
            'add_button_classes' => '',
            'load_more_button_route' => 'bottler',
            'source' => 'list',
        ];
        ...
        $page += [
            ...
            'add_modal_create_url' => url('bottler/create'),
            'add_modal_action_url' => url('bottler'),
        ...
        if ($section == 'bottler') {
            $page += [
                'heading' => __('lang.bottler'),
                'meta_title' => __('lang.bottler'),
            ];
