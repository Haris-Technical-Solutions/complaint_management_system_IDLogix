...
class CadIndex {
    ...
    public function handle($request, Closure $next) {
        ...
        if (!config('visibility.modules.cads')) {
           // abort(404);
            return $next($request);
        }
    ...
    function toggleOwnFilter() {
        ...
        if (request('toggle') == 'pref_filter_show_archived_projects') {
            ...
            auth()->user()->save();
        }else if (request('toggle') == 'pref_filter_show_active_projects') {
            auth()->user()->pref_filter_show_archived_projects = 'no';
        auth()->user()->save();
        }else if (request('toggle') == 'pref_filter_show_all_projects') {
            auth()->user()->pref_filter_show_archived_projects = 'all';     
            auth()->user()->save();
        }else
        {
                 //toggle database settings
        ...
        }else if (auth()->user()->pref_filter_show_archived_projects == 'no') {
            request()->merge(['filter_show_active_projects' => 'yes']);
        }else if (auth()->user()->pref_filter_show_archived_projects == 'all') {
            request()->merge(['filter_show_all_projects' => 'yes']);
        }
