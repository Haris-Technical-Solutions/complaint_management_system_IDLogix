...
class Tasks extends Controller {
    ...
    private function statsWidget($data = array()) {
        ...
        foreach (config('task_statuses') as $status) {
            ...
            $stat = [
                'value' =>  $this->statsrepo->countTasks([
                    'task_status' =>  $status->taskstatus_id,
                    'client_id' => auth()->user()->clientid,
                    'assigned' => auth()->user()->role_id==16?auth()->id():null,
                    'project_manager' => auth()->user()->role_id==17?auth()->id():null,
                ]),
