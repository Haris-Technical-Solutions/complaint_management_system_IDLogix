...
use Validator;
/////////////////////
use App\Http\Responses\Tasks\ShowCalanderResponse;
  ...
  /**
    ...
    public function getCustomFields($obj = '') {
        ...
        request()->merge([
            ...
            //Sort custom fields in Table : by Sher Ali 
            'sort_by' => 'customfields_position',
        ]);
...
public function showAttachments(AttachmentRepository $attachmentrepo, $id) {
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        if ($section == 'tasks') {
            $page += [
                ...
                'heading' => ($data['worklog']??'').' '. __('lang.tasks'),
                'mainmenu_tasks' => 'active',
            ];
            return $page;
        }
        if ($section == 'calander') {
            if(request('calander')=='my'){
                $page += [
                    'meta_title' => __('My Calendar'),
                      'heading' =>  __('My Calendar'),
                    'mainmenu_calander' => 'active',
                ];
                
            }else{
                $page += [
                    'meta_title' => __('Team Calendar'),
                      'heading' =>  __('Team Calendar'),
                    'mainmenu_calander' => 'active',
                ];
            }
            return $page;
     ...
     * @param  int  $id
     ...
     public function showMyCalander( TaskAssignedRepository $assignedrepo) {
       ...
       //  'filter_assigned'=>[auth()->id()],
        ...
        $parent_tasks = array();
        $child_tasks = array();
           $cal_start_date = Carbon::parse($tasks->min("task_date_start"));
          $cal_end_date = Carbon::parse($tasks->min("task_date_due"));
        //apply some permissions
        // return request('filter_assigned');
        
         foreach(config('system.team_members') as $user)
            {
                if(@request('calander')=='my' && $user->id!=auth()->id())
                    continue;
                
               
                    if(request('filter_assigned') ==null || in_array($user->id, request('filter_assigned')))
                    {
                      $cal_task = new \App\Models\CalanderTask();
                      $cal_task->id = "".(1000000 + $user->id);
                      $cal_task->user_id = $user->id;
                      ...
                      //  $cal_task->duration = 0.01;
                        ...
                        //  $cal_task->start_date = '2022-11-22';
                        $cal_task->owner = $user->first_name.' '.$user->last_name ;
                         $cal_task->color=$user->user_color??sprintf("#%06x", rand(0, 16777215));
                         $cal_task->borderColor=sprintf("#%06x", rand(0, 16777215));
                         $cal_task->backgroundColor=$cal_task->color;
                         $cal_task->dragBackgroundColor=$cal_task->borderColor;
                                
                        $caltasks[] = $cal_task;
                        $parent_tasks[] = $cal_task;  
        ...
        if ($tasks) 
        {
            ...
            foreach ($tasks as $task) 
            {
                $this->applyPermissions($task);
                $cal_task = new \App\Models\CalanderTask();
                $cal_c_task = new \App\Models\CalanderTask();
                
                $cal_task->id = $task->task_id;
                $cal_task->user_id = $task->tasksassigned_userid;
               
                $cal_task->text = $task->task_title;
                $toDate = Carbon::parse($task->task_date_start);
                $fromDate = Carbon::parse($task->task_date_due);
  
                $days = $toDate->diffInDays($fromDate) ;
               
                $cal_task->duration = $days +1;
                $cal_task->progress = 0;
                $cal_task->start_date = $task->task_date_start;
                $cal_task->owner = $task->first_name;
                $cal_task->project = $task->project_title;
               
                $cal_c_task->id = $task->task_id;
                $cal_c_task->title = $task->project_title .' ('.$task->task_title.')';
                // $cal_c_task->category = $task->first_name;
                $cal_c_task->body = $task->task_description;
                $cal_c_task->start = $task->task_date_start;
                $cal_c_task->end = $task->task_date_due;
                $lead = $task->project->lead;
               
                $cal_c_task->location =  $lead->lead_city ?? $lead->lead_state ?? $lead->lead_zip ?? $lead->lead_street ?? 'N/A'  ;
                $cal_c_task->state = "";
                // return $task;
                // $full_names = $assignedrepo->getAssigned($task->task_id)->where('type','team');//->pluck('first_name','last_name','avatar');
                $assigned = array();
                  foreach ($task->assigned as $user) {
                    $img = '';
                    if($user->avatar!='')  
                      $img = '<img height="20px" width="20px" src="'.$user->avatar .'" id="topnav_avatar" alt="user" class=""> ';
                      
                	$assigned[]= $img.$user->first_name .' '. $user->last_name ;
                }

                
                  
                $cal_c_task->attendees =$assigned ;
                $cal_c_task->raw = $task->project->project_title;
                $cal_c_task->isReadOnly=true;
                
                
               
             
                foreach($parent_tasks as $user)
                    {
                        if($task->tasksassigned_userid == $user->user_id){
                          
                            $cal_task->parent = 1000000 + $user->user_id;
                            $cal_task->owner = '';
                            $cal_task->color = $user->color;
                            // $cal_task->calendarId = "".(1000000 + $user->user_id);
                            $cal_c_task->calendarId = "".(1000000 + $user->user_id);
                            $cal_c_task->backgroundColor = $user->color;
                            $cal_c_task->color = '#fff';
                ...
                // if($project->category)    
                    // $cal_task->color = $project->category->category_color;
                //  else
                //  $cal_task->color ='';
                 if($cal_task->parent==0){
                     $cal_task->parent = 999999;
                     $cal_c_task->calendarId = "999999";
                     $is_unassigned = true;
                 }
                $caltasks[] = $cal_task;
                // if($cal_c_task->id==6)
                $child_tasks[] = $cal_c_task;
                
            }
                
        }
        request()->merge([
        'orderby' => 'leave_start_date',
        'sortorder' => 'asc',
         'per_page' => 'all',
    ...
    //   return request('orderby');
        if ($leaves) 
        {
            ...
            foreach ($leaves as $leave) {
                     $cal_task = new \App\Models\CalanderTask();
                        ...
                        $cal_task->id = $leave->leave_id;
                        $cal_task->user_id = $leave->leave_requester_userid;
                       
                        $cal_task->text = $leave->leave_title.'('.$leave->leave_type.')';
                        $toDate = Carbon::parse($leave->leave_start_date);
                        $fromDate = Carbon::parse($leave->leave_end_date);
          
                        $days = $toDate->diffInDays($fromDate) ;
                       
                        $cal_task->duration = $days +1;
                        $cal_task->progress = 0;
                        $cal_task->start_date = $leave->leave_start_date;
                        $cal_task->owner = $user->first_name.' '.$user->last_name;
                        $cal_task->project = $leave->leave_type;
                       
                        $cal_c_task->id = $leave->leave_id;
                        $cal_c_task->title = $leave->leave_title.'('.$leave->leave_type.')' ;
                        // $cal_c_task->category = $task->first_name;
                        $cal_c_task->body = $leave->leave_type;
                        $cal_c_task->start = $leave->leave_start_date;
                        $cal_c_task->end = $leave->leave_end_date;
                        // $lead = $leave->project->lead;
                        ...
                        $cal_c_task->attendees =$assigned ;
                        $cal_c_task->raw = '';//$leave->project->project_title;
                        $cal_c_task->isReadOnly=true;
                        ...
                        foreach($parent_tasks as $user)
                            {
                                if($leave->leave_requester_userid == $user->user_id){
                                  
                                    $cal_task->parent = 1000000 + $user->user_id;
                                    $cal_task->owner = '';
                                    $cal_task->color = 'FFFF99';//$user->color;
                                    // $cal_task->calendarId = "".(1000000 + $user->user_id);
                                    $cal_c_task->calendarId = "".(1000000 + $user->user_id);
                                    $cal_c_task->backgroundColor = '#FFFF99';//$user->color;
                                    $cal_c_task->color = '#FF3333';
        ...
        if($is_unassigned){
            ...
            $cal_task->text = 'UnAssigned';
            $cal_task->body = $cal_task->text;
            $cal_task->owner = 'UnAssigned';
            $caltasks[] = $cal_task;
            // $child_tasks[] = $cal_task;
            $parent_tasks[] = $cal_task;
        ...
        //basic page settings
        $page = $this->pageSettings('calander', ['count' => $count,'worklog'=>$worklog]);

        //page setting for embedded view
        if (request('source') == 'ext') {
            $page = $this->pageSettings('ext', ['count' => $count]);
        }
        $page['mainmenu_calander']='active';
...
//  return request('action');
        ...
        config(['visibility.showcalander' => true]);
        ...
        // return 'asd';
            ...
            return new ShowCalanderResponse($payload);
