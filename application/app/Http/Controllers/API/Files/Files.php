...
class Files extends Controller {
    ...
    protected $filerepo;
    /**
     * The filegroup repository instance.
     */
    ...
    public function index() {
        ...
        if(request("fileresource_type")=='project'){
            $project =\App\Models\Project::find( request('fileresource_id'));
            // $folders =\App\Models\FileGroup::where( 'file_group_resource_id',request('fileresource_id'))->get();
            $folders = $this->filegrouprepo->search();
            if(request('filter_file_clientid'))
            {
                $prj_qry =\App\Models\Project::where( 'project_clientid',request('filter_file_clientid'));
                
            if(request('search_query')){
                $prj_qry->where('project_title','like %'.request('search_query').'%');
            }
                $project_ids=$prj_qry->get()->pluck('project_id');
            }
            
            if($project || isset($project_ids)){
                //attachments
                request()->merge([
                ...
                if($project){
                request()->merge([
                    'attachmentresource_id' => $project->project_created_from_leadid,
                    ]);
                }

                if(isset($project_ids)){
                request()->merge([
                    'attachmentresource_ids' => $project_ids,
                    ]);
                }
                
                $attachments = $this->attachmentrepo->search();
                foreach ($attachments as $attachment) {
                   // $leadController->applyAttachmentPermissions($attachment);
                }
                $payload['lead_files'] =$attachments;
                $payload['folders'] =$folders;
            } 
            else{
            $payload['lead_files'] =array();
        ...
        if(request('filegroup_id')){
             $payload['filegroup_id'] =request('filegroup_id');
            $payload['filegroup'] = \App\Models\FileGroup::find(request('filegroup_id'));
            
        }else{
            $payload['filegroup'] = array();
        }
           return $payload; 
        //show the view
    ...
    public function store() {
        ...
        //validation
        if (request('filegroup_id')==null) {
            //error
            abort(409, __('Folder not selected. please select folder'));
        }
        ...
        if (request()->filled('attachments')) {
            // foreach (request('attachments') as $uniqueid => $file_name) {
                ...
                if(request('filegroup_id')){
                    $data['file_group_id'] = @request('filegroup_id');  
        ...
        $payload['fileresource_id'] = request('fileresource_id');
        $payload['fileresource_type'] = request('fileresource_type');
        return $payload;
        //show the view
    ...
    public function moveFile() {
        ...
        //get the item
        $file = \App\Models\File::Where('file_id', request('file_id'))->first();
        ...
        $file->file_group_id = $new_folder;
        $file->save();

        $jsondata['notification'] = [
                'type' => 'success',
                'value' =>  __('File moved successfully.'),
            ];
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        $page = [
            ...
            'dynamic_search_url' => url('files/search?action=search&fileresource_id=' . request('fileresource_id') . '&fileresource_type=' . request('fileresource_type').$searchtype),
        ...
        $page += [
            ...
            'add_modal_create_url' => url('files/create?'.$url_param),
            'add_modal_action_url' => url('files?'.$url_param)  ,
