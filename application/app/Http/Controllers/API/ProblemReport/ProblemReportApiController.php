...
namespace App\Http\Controllers\API\ProblemReport;
...
class ProblemReportApiController extends Controller {
    ...
    // return request();
        //get tickets
         request()->merge(['filter_my_ticket' => array(auth()->id())]);
    ...
    public function create(CategoryRepository $categoryrepo) {
        ...
        if (auth()->user()->is_client) {
            ...
            if(request('ticketresource_type')=='project')
            {
                $clients_projects = \App\Models\Project::Where('project_id', request('ticketresource_id'))->get();
            }
            else
                $clients_projects = [];
        }
        ...
        if (request()->filled('attachments')) {
            // foreach (request('attachments') as $uniqueid => $file_name) {
                $data = [
                    ...
                    'attachment_directory' => request('uniqueid'),
                    'attachment_uniqiueid' => request('uniqueid'),
                    'attachment_filename' => request('file_name'),
                ];
    ...
    public function edit(CategoryRepository $categoryrepo, $id) {
       ...
       $tickets = $this->ticketrepo->search($id);
        // return $tickets[0]->ticket_client_visibility;
        //client categories
        $categories = $categoryrepo->get('ticket');
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        if(request('ticketresource_id')){
        $url = $url.'?ticketresource_id=' . request('ticketresource_id') . '&ticketresource_type=' . request('ticketresource_type');
        }else{

             config(['visibility.list_page_actions_add_button' => false,
             'visibility.list_page_actions_add_button_link' =>false,
             ]);
        }
    
        $page += [
            'add_button_link_url' => url($url),
        ];
