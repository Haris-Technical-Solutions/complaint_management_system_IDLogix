...
use App\Http\Responses\Projects\CadDetailsResponse;
use App\Http\Responses\Projects\EditResponse;
use App\Http\Responses\Projects\ContentResponse;
use App\Http\Responses\Projects\TeamResponse;
...
use App\Http\Responses\Projects\Views\CadListResponse;
use App\Http\Responses\Projects\Views\CadHistoryListResponse;
...
class ProjectController extends Controller {
    ...
    public function __construct(
       ...
       // Permissions on methods
        ...
        //Permissions on methods
        $this->middleware('cadsMiddlewareIndex')->only([
            'cad_jobs'
        ]);
        

         $this->middleware('cadsMiddlewareShow')->only([
           'cadShow',
              'cad_details',
         ]);

         $this->middleware('cadsMiddlewareEdit')->only([
              'cad_details',
         ]);
    ...
    public function show(TimerRepository $timerrepo, $id) {
        ...
        $jobtype = $this->getCustomField($project,'Job Type')->first();
    ...
    public function showDynamic($id) {
        ...
        //apply permissions
        $this->applyPermissions($project);
            // $project->permission_edit_project ;
            //delete permissions
            // $project->permission_delete_project
        ...
        case 'teams':
            ...
            $page['dynamic_url'] = url('projects/' . $project->project_id . '/' .$section . '?source=ext&' . $section . 'resource_type=project&' . $section . 'resource_id=' . $project->project_id);
            break;
        case 'locations':
            $sections = request()->segment(3);
            $section = rtrim($sections, 's');
            $page['dynamic_url'] = url('leads/content/' . $project->project_created_from_leadid . '/show-map?ref=detail');
            break;
        case 'logs':
            $sections = request()->segment(3);
            $section = rtrim($sections, 's');
        ...
        case 'details':
            ...
            break;
        case 'cad-info':
            $page['mainmenu_cadscreen']='active';
            $page['mainmenu_projects']='';
            $page['dynamic_url'] = url('projects/' . $project->project_id . '/cad-details');
            break;
        default:
    ...
    //   return $request;
        ...
        $project = $projects->first();
         ...
         if($project->project_categoryid==1 && count($newly_signed_users)>0){
            $project->project_categoryid = 81;
            $project->save();
        }
    ...
    public function getCustomFields($obj = '') {
        ...
        if ($obj instanceof \App\Models\Project) {
            foreach ($fields as $field) {
                if($obj->project_status=='not_started' && ($field->customfields_title=='PASCA Sent' or $field->customfields_title=='PASCA Approved'))
                    $field->customfields_datatype='';
                else if($obj->project_status=='pasca_sent' && ($field->customfields_title=='PASCA Approved'))
                    $field->customfields_datatype='';
                else
     ...
     public function getCustomField($obj = '',$fieldtitle='') {

        //set typs
        request()->merge([
            'customfields_type' => 'projects',
            'filter_show_standard_form_status' => '',
            'filter_field_status' => 'enabled',
            'sort_by' => 'customfields_position',
            'customfields_title' =>$fieldtitle,
        ]);
    ...
    public function changeStatusUpdate() {
        ...
        //update the project
        $project->project_status = @request('project_status')==''?'not_started':request('project_status');
        
        if(request('project_status')=='sent_to_cad' && (@$project->project_custom_field_15 =='' || @$project->project_custom_field_15 =='---'))
            {
                $project->project_custom_field_15=date('Y-m-d');
                
                 $pcat = \App\Models\Project::groupBy('projects.project_categoryid');
                     $pcat->selectRaw('round(Avg(DATEDIFF(project_custom_field_18 , project_custom_field_15))) cad_total_days');
                     $pcat->where('projects.project_categoryid',$project->project_categoryid);
                      $pcat = $pcat->get();
                    
                    $cad_days=15;
                    if($pcat->count()>0){
                        $cad_days = $pcat[0]->cad_total_days;
                    }
                     if(@$project->project_custom_field_17=='')
                        $project->project_custom_field_17=date('Y-m-d', strtotime("+".$cad_days." days"));
                    
                     $project->project_custom_field_41 = 'Not Started';
            }
        if($project->project_status=='cancelled' || $project->project_status =='completed' || $project->project_status =='payment_received')        
            $project->project_active_state = 'archived';
        else
            $project->project_active_state = 'active';
 ...
 /**
    ...
    public function changeCadStatusUpdate(CategoryRepository $categoryrepo) {
        ...
        $project = \App\Models\Project::Where('project_id', request()->route('project'))->first();
        //old status
        $old_status = $project->project_custom_field_41;

        //validate
        if (!in_array(request('project_cad_status'), ['not_started', 'in_progress', 'on_hold', 'cancelled', 'completed'])) {
          //  abort(409, __('lang.invalid_status'));
        }

        //update the project
        $project->project_custom_field_41 = @request('project_cad_status')==''?'Not Started':request('project_cad_status');
      
        $project->save();

        //get refreshed project
         request()->merge([
            'filter_type'=>'cad_jobs',
        ]);
        ...
        $data = [
            ...
            'event_item_id' => '',
            'event_item_lang' => 'event_changed_project_status',
            'event_item_content' => $project->project_custom_field_41,
            'event_item_content2' => '',
            'event_parent_type' => 'project',
            'event_parent_id' => $project->project_id,
            'event_parent_title' => $project->project_title,
        ...
        //record event
        if ($old_status != request('project_cad_status')) {
            if ($event_id = $this->eventrepo->create($data)) {
                //get users
                $users = $this->projectpermissions->check('users', $project);
                //record notification
        ...
        $tags = $this->tagrepo->getByType('project');
        $page = $this->pageSettings('projects');
        $page["crumbs"] =[__('CAD Jobs')];
        $page["heading"] =__('CAD Jobs');
        $page['mainmenu_cadscreen']='active';
        $page['mainmenu_projects']='';
        //reponse payload
        $payload = [
        ...
        //show the form
           return new CadListResponse($payload);
        // $this->index($categoryrepo);
    }
    /**
    ...
    //   return request('project_custom_field_53');
        //save
        $project->save();
        // return $this->projectrepo->update( $project->project_id);
        //update
        if (!$this->projectrepo->update( $project->project_id)) {
            abort(409);
        }
        //delete & update tags
        ...
        //client categories
        $categories = $categoryrepo->get('project');
        $jobtypes = $this->getCustomField($project,"Job Type")->first();
       
        //reponse payload
        $payload = [
            'page' => $this->pageSettings('edit'), // $this->pageSettings('project', $project),
            'project' => $project,
            'tags' => $tags,
            'contacts' =>$contacts,
            'fields' => $this->getCustomFields($project),
        ...
        if(request('cad_form'))
            return $this->cad_details($categoryrepo,$project->project_id) ;
        else
    ...
    public function assignedUsersUpdate(ProjectAssignedRepository $assignedrepo,ProjectManagerRepository $managerrepo, $id) {
        ...
        if (request('assigned') ==null && $project->project_categoryid>1  ) {
            abort(409,'Team Must be Assigned');
        }
       ...
       if($project->project_categoryid==1 && request('assigned') && request('movetoworklog')==1 ){
            $project->project_categoryid = 81;
             }
              //update client project manager
               if(request('project_custom_field_51'))
                   $project->project_custom_field_51 = request('project_custom_field_51');
                
                //update site manager
                if(request('project_custom_field_52'))
                    $project->project_custom_field_52 = request('project_custom_field_52');
    ...
    private function pageSettings($section = '', $data = []) {
        ...
        $url = 'projects/search?action=search&projectresource_id=' . request('projectresource_id') . '&projectresource_type=' . request('projectresource_type') . '&filter_category=' . request('filter_category');
        if(request('worklog'))
        $url = $url.'&worklog=' . request('worklog');
        //common settings
        $page = [
            'crumbs' => [
                __('lang.projects'),
        ...
        if (request()->filled('worklog')) {
                    $page['crumbs'][]=__('lang.task_worklog');            
        }
        if (request()->filled('filter_category')) {
            if ($category = \App\Models\Category::Where('category_type', 'project')->where('category_id', request('filter_category'))->first()) {
                ...
                $page['crumbs'][]=$category->category_name;
            }
        }
 ...
 public function activity_log($id) {
          ...
          $logs =  \App\Models\Event::where('event_parent_id', $id)
          ->with('creator')
        ->where('event_parent_type', 'project')
    ...
    public function team($id) {
        ...
        //permission
        // if (auth()->user()->role->role_assign_projects != 'yes' && auth()->user()->is_client == false) {
        //      abort(403);
        // }
       ...
       $projects = $this->projectrepo->search($id, ['apply_filters' => false]);
        ...
        // auth()->user()->is_manager ||
                    if ($this->projectpermissions->check('edit', $project) && (auth()->user()->is_manager || auth()->user()->is_management || auth()->user()->is_admin)){
                        config([
      ...
      return  $payload = [
            // 'page' => $this->pageSettings('edit'),
            // 'project' => $project,
            'job_owner'=>isset($manager)?implode (", ", $manager):'',
            'team'=>isset($job_assigned)?implode (", ", $job_assigned):'',
            'project_manager'=>\App\Models\User::find($project->project_custom_field_51),
            'site_manager'=>\App\Models\User::find($project->project_custom_field_52),
            'qoute_responsible'=>isset($assigned)?implode (", ", $assigned):'',
            'contacts' =>$contacts,
         ];
        //response
 ...
 /**
    ...
    public function cad_jobs_history(CategoryRepository $categoryrepo,IBankHoliday $bankHoliday) {
         ...
         //validate module status
        //team
  return    $holidays = UkBankHolidayFactory::getAll();
    // $bankHolidays = $bankHoliday->getAll(\Midnite81\BankHolidays\Enums\Territory::ENGLAND_AND_WALES);
        ...
        //get team projects
        // $projects = $this->projectrepo->search();
        $projects = $this->projectrepo->cadHistory();
        ...
        $tags = $this->tagrepo->getByType('project');
        $page = $this->pageSettings('projects');
        $page["crumbs"] =[__('CAD Jobs History')];
        $page["heading"] =__('CAD Jobs History');
        $page['mainmenu_cadhistory']='active';
        $page['mainmenu_projects']='';
        //reponse payload
        $payload = [
            'page' => $page,
        ...
        switch (auth()->user()->pref_view_projects_layout) {
        case 'list':
            return new CadHistoryListResponse($payload);
        case 'card':
            return new CardResponse($payload);
    ...
    public function cad_details(CategoryRepository $categoryrepo,$id) {
         ...
         if (auth()->user()->is_team) {
            if (auth()->user()->role->role_cads < 1) {
            config(['visibility.modules.cad' => true]);
            abort(404, __('lang.the_requested_service_not_found'));
            return $next($request);
            }
        }
        ...
        //return runtimePreselectedInArray(auth()->id() ?? '', json_decode($project->project_custom_field_53) ?? []);
        //return json_decode($project->project_custom_field_53);
        ...
        $contacts = \App\Models\User::where('clientid', $project['project_clientid'])->where('type', 'client')->get();
        $fields = $this->getCustomFields($project);
        $jobtype = $this->getCustomField($project,"CAD Status")->first();
        $bim_statuses = $this->getCustomField($project,"BIM Status")->first();
        $topo_qa_approval = $this->getCustomField($project,"Topo QA Approval")->first();
        $topo_report = $this->getCustomField($project,"Topo Report")->first();
        $topo_survey = $this->getCustomField($project,"Topo Survey")->first();
        $utility_qa_approval = $this->getCustomField($project,"Utility QA Approval")->first();
        $utiltiysurveyreport = $this->getCustomField($project,"Utility Survey & Report")->first();
        
        $page = $this->pageSettings('edit');
        
        $page["crumbs"] ='CAD Jobs';
        $page["heading"] ='CAD Jobs';
        $page['mainmenu_cadscreen']='active';
        $page['mainmenu_projects']='';
        //reponse payload
        $payload = [
            ...
            'fields' => $fields,
            'cadstatuses' =>$jobtype,
            'bim_statuses' =>$bim_statuses,
            'topo_qa_approval' =>$topo_qa_approval,
            'topo_report' =>$topo_report,
            'topo_survey' =>$topo_survey,
            'utility_qa_approval' =>$utility_qa_approval,
            'utiltiysurveyreport' =>$utiltiysurveyreport,
        ];
        //response
      
         return new CadDetailsResponse($payload); 
    //return new ShowDynamicResponse($payload);
    }
