...
/** --------------------------------------------------------------------------------
 * This classes renders the response for the [index] process for the leaves
 * controller
 * @package    Grow CRM
 * @author     NextLoop
 *----------------------------------------------------------------------------------*/
...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/leaves/components/table/ajax';
                $dom_container = '#leaves-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/leaves/components/table/table';
                $dom_container = '#leaves-table-wrapper';
                $dom_action = 'replace-with';
                break;

            //template and dom - for ajax initial loading
            default:
                $template = 'pages/leaves/tabswrapper';
            ...
            if ($leaves->currentPage() < $leaves->lastPage()) {
                $url = loadMoreButtonUrl($leaves->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'leaves','leaves_withtypes'))->render();
            ...
            $jsondata['dom_html'][] = [
                ...
                'action' => 'replace',
                'value' => strtoupper(__('lang.leaves')),
            ];
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($leaves->currentPage() + 1, request('source'));
            $page['loading_target'] = 'leaves-td-container';
            $page['visibility_show_load_more'] = ($leaves->currentPage() < $leaves->lastPage()) ? true : false;
            return view('pages/leaves/wrapper', compact('page', 'leaves','leaves_withtypes'))->render();
        }
