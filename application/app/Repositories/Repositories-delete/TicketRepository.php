...
class TicketRepository {
    ...
    public function search($id = '', $data = array()) {
        ...
        //filter: date (end)
        if (isset($data['not_status'])) {
            $tickets->where('ticket_status', '!=', $data['not_status']);
        }
        //filter: date (end)
        ...
        if (request()->filled('filter_ticket_client_visible')) {
            $tickets->where('ticket_client_visibility', request('filter_ticket_client_visible'));
        }
        
        
          //filter creator_id
        if (request()->filled('filter_my_ticket')) {
            $tickets->where('ticket_creatorid', request('filter_my_ticket'));
        }
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            case 'category':
                ...
                break;
            case 'project':
                $tickets->orderBy('projects.project_title', request('sortorder'));
                break;
            case 'team':
                $tickets->orderBy('users.first_name', request('sortorder'));
                break;
            }
        } else {
            //default sorting
    ...
    public function create() {
        ...
        $ticket->ticket_client_visibility = request('ticket_client_visibility');
        //save and return id
        if ($ticket->save()) {
