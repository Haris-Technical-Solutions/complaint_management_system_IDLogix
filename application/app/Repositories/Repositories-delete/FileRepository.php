    ...
    public function search($id = '', $data = []) {
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('fileresource_type')) {
                ...
                if(request('fileresource_type')=='project'){
                    ...
                    $project = Project::find( request('fileresource_id'));
                              
                     $files->where(function($qery)  use ($project)
                            {
                             
                                $qery->where(function($qry) {
                                        $qry->where('fileresource_type', request('fileresource_type'));
                                        $qry->where('fileresource_id', request('fileresource_id'));
                                
                                })
                                ->whereOr(function($qry) use ($project)
                                    {
                                        $qry->where('fileresource_type', 'lead')
                                        ->where('fileresource_id', $project->project_created_from_leadid);
            ...
            //filters: filegroup_id
            if (request()->filled('filegroup_id')) {
                $files->where('file_group_id', request('filegroup_id'));
            }
            //filters: client
        ...
        if (request()->filled('search_query') || request()->filled('query')) {
            ...
            $files->where(function ($query) {
                ...
                $query->orWhere('client_company_name', 'LIKE', '%' . request('search_query') . '%');
                $query->orWhere('users.first_name', 'LIKE', '%' . request('search_query') . '%');
                // $query->orWhere('projects.project_title', 'LIKE', '%' . request('search_query') . '%'); 
