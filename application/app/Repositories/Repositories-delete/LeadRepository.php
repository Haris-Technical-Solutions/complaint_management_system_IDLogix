...
class LeadRepository {
    ...
    public function search($id = '') {
        ...
        if (!is_numeric($id)) {
            ...
            if (request()->filled('filter_show_active_leads') ) 
            {
                $leads->where('lead_active_state', 'active');
            }else if (request()->filled('filter_show_archived_leads') ) 
            {
                $leads->where('lead_active_state', 'archived');
            }
            else if (request()->filled('filter_show_all_leads') ) 
            {
                // $leads->where('lead_active_state', 'archived');
            }else{
        ...
        if (request()->filled('search_query') || request()->filled('query')) {
            $leads->where(function ($query) {
                ...
                $query->orWhere('leadstatus_title', '=', request('search_query'));
                 $query->orWhere('category_name', 'like','%'.request('search_query').'%');
                $query->orWhere('lead_custom_field_4', 'like','%'.request('search_query').'%');
    ...
    public function create($position = '') {
        ...
        if(request('lead_converted_clientid')){
        $lead->lead_converted_clientid = request('lead_converted_clientid');
        $lead->lead_converted = 'yes';
        $lead->lead_converted_date = date('Y-m-d');
        $client =  \App\Models\Client::find(request('lead_converted_clientid'));
        $lead->lead_company_name = $client->client_company_name;
        }else if(request('lead_company_name')!='')
        {
         /** ----------------------------------------------
         * create the client
         * ----------------------------------------------*/
            $client = new \App\Models\Client();
            $client->client_company_name = request('lead_company_name');
            $client->client_import_first_name = request('lead_firstname');
            $client->client_import_last_name = request('lead_lastname');
            $client->client_import_email = request('lead_email');
            ...
            $lead->lead_converted_clientid = $client->client_id;
            $lead->lead_converted = 'yes';
            $lead->lead_converted_date = date('Y-m-d');
           //save new client
        ...
        if(@request('lead_custom_field_36')!='')
            $lead->lead_custom_field_36 = request('lead_custom_field_36');
        else
            $lead->lead_custom_field_36 =date('Y-m-d H:i:s',strtotime(date('Y-m-d h:i:s').' +7 day'));  
    ...
    public function update($id) {
        ...
        $lead->lead_last_contacted = request('lead_custom_field_38');
