...
class ProjectRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        $projects->leftJoin('leads', 'projects.project_created_from_leadid', '=', 'leads.lead_id');
        ...
        if (!is_numeric($id)) {
            ...
            if ( request()->filled('filter_show_active_projects') ) 
            {
                $projects->where('project_active_state', 'active');
            }
            else if (  request()->filled('filter_show_archived_projects') ) 
            {
                 $projects->where('project_active_state', 'archived');
            }else if (request()->filled('filter_show_all_projects') ) 
            {
        ...
        // return request();
         //[data filter] - client's Project/Site Manager
        if (request()->filled('project_client_projectmanager') ||request()->filled('project_client_sitemanager')) {
            // return  request('project_client_projectmanager');
            $projects->where(function($qry){ 
                $qry->where('project_custom_field_51','=',  request('project_client_projectmanager'));
                 $qry->orWhere('project_custom_field_52','=', request('project_client_sitemanager'));
                // $qry->orWhere('project_status','=', 'issue_to_client');
        ...
        if ($data['apply_filters']) {
            ...
            if (request()->filled('filter_category')) {
                ...
                if(request('filter_category')==1){
                    $projects->where('project_categoryid', 1);
                    // doesntHave('assigned');
                    //$projects;
                }else{
            ...
            if (is_array(request('filter_tags')) && !empty(array_filter(request('filter_tags')))) {
             ...
             //[data filter] cad_project
                if ( request()->filled('filter_type')) {
                    $projects->where(function($qry){ 
                        $qry->where('project_status','=', 'sent_to_cad');
                        $qry->orWhere('project_status','=', 'cad_complete');
                        // $qry->orWhere('project_status','=', 'issue_to_client');
            ...
            $projects->where(function ($query) {
                ...
                $query->orWhereHas('assigned', function ($q) {
                 ...
                 $query->orWhereHas('lead', function ($q) {
                    $q->where('lead_city', '=', request('search_query'));
                });
                 $query->orWhere('project_custom_field_44', 'like',  '%' .request('search_query'). '%');
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            case 'project_address':
                $projects->orderBy('leads.lead_city', request('sortorder'));
                break;
            }
        } else {
            //default sorting
    ...
    public function update($id) {
        ...
        if(request('project_categoryid'))
        $project->project_categoryid = request('project_categoryid');
        
        if(request('project_date_start'))
        $project->project_date_start = request('project_date_start');
        
        if(request('project_date_due'))
        $project->project_date_due = request('project_date_due');
        ...
        if ($project->save()) {
            ...
            if($project->project_custom_field_32=='on' && $project->project_active_state!='archived')
                {
                    $project->project_active_state='archived';
                    $project_update = true;
                }else if($project->project_custom_field_32=='off' && $project->project_active_state!='active')
                {
            ...
            if($project->project_status=='sent_to_cad' )
                {
                    
                    $pcat = $this->projects->newQuery();
                     $pcat->groupBy('projects.project_custom_field_44');
                     $pcat->selectRaw('round(Avg(DATEDIFF(project_custom_field_18 , project_custom_field_15))) cad_total_days');
                     $pcat->where('projects.project_custom_field_44',$project->project_custom_field_44);
                     $pcat = $pcat->get();
                    ...
                    if(@$project->project_custom_field_15=='')
            ...
            if($project->project_status=='cad_complete' && $project->project_custom_field_18=='')
                {
                    $project->project_custom_field_18=date('Y-m-d');
                    $project_update = true;
                }
                
            if(@request('project_custom_field_44')!='')
                {
                    $project->project_custom_field_44=request('project_custom_field_44');
                    $project_update = true;
                }
                 if(@request('project_custom_field_16')!='')
                {
                   
                    $project_update = true;
                }
                
                if(request('project_custom_field_41'))
                {
                    if(request('project_custom_field_41')=='CAD Complete')
                        $project->project_status='cad_complete';
                        
                     $project->project_custom_field_18=date('Y-m-d');
                    $project->project_custom_field_41 = request('project_custom_field_41');
                    $project_update = true;
                }
                
                if(@request('project_custom_field_33')=='on')
                {
                    $project->project_custom_field_33='yes';
                     $project->project_custom_field_16=request('project_custom_field_16');
                     
                        $project->project_status='issue_to_client';
                        $project->project_custom_field_18=date('Y-m-d');
                    $project_update = true;
                }else
                {
                      $project->project_custom_field_33='no';
                       $project->project_custom_field_16=null;
                      //  $project->project_status='cad_complete';
                }
            
              if(@request('project_custom_field_34')=='on')
                {
                    $project->project_custom_field_34='yes';
                     $project->project_custom_field_21=request('project_custom_field_21');
                    $project_update = true;
                }else
                {
                    $project->project_custom_field_34='no';
                     $project->project_custom_field_21='';
                    $project_update = true;

                }
                
                if(@request('project_custom_field_17')!='')
                {
                     $project->project_custom_field_17=request('project_custom_field_17');
                    $project_update = true;
                }else
                {
                     $project->project_custom_field_17='';
                    $project_update = true;

                }
            
             if(@request('project_custom_field_5')!='')
                {
                     $project->project_custom_field_5=request('project_custom_field_5');
                    $project_update = true;
                }else
                {
                     $project->project_custom_field_5='';
                    $project_update = true;
                ...
                if(@request('project_custom_field_72')!='')
                {
                     $project->project_custom_field_72=request('project_custom_field_72');
                    $project_update = true;
                }
            
            if(@request('project_custom_field_73')!='')
                {
                     $project->project_custom_field_73=request('project_custom_field_73');
                    $project_update = true;
                }
                
                if(@request('project_custom_field_53')!='')
                {
                    // return request('project_custom_field_53');
                      $project->project_custom_field_53=request('project_custom_field_53');
                    $project_update = true;
                }
            if($project_update == true)
                $project->save();
    ...
    public function assignCategoryProjects($category_id = '') {
     ...
     public function cadHistory() {
     $projects = $this->projects->newQuery();
     $projects->leftJoin('categories', 'categories.category_id', '=', 'projects.project_categoryid');
    //  $projects->orderBy('category_name');
     $projects->groupBy('project_custom_field_44');
     $projects->select('project_custom_field_44 as category_name');
     $projects->selectRaw('count(distinct projects.project_id) as total_projects');
     $projects->selectRaw('round(Avg(DATEDIFF(project_custom_field_18 , project_custom_field_15))) cad_total_days');
     // $projects->whereIn('categories.category_id',[76,77,78,87]);
      $projects->where('project_custom_field_44','!=','');
