    ...
    public function countTasks($data = []) {
        ...
        if (isset($data['status']) && $data['status'] != '') {
            if ($data['status'] == 'pending') {
                $tasks->whereIn('task_status', [7]);
            } else {
                $tasks->whereIn('task_status', $data['status']);
            }
        }
...
/**
     ...
     * @return float
     */
    public function countLeaves($data = []) {
        ...
        //status
        if (isset($data['leave_status']) && $data['leave_status'] != '') {
            if ($data['leave_status'] == 'pending') {
                $tasks->whereIn('leave_status', ['Pending']);
            } else {
                $tasks->whereIn('leave_status', $data['leave_status']);
            }
        }
        ...
        if (isset($data['assigned']) && is_numeric($data['assigned'])) {
            request()->merge(['for_assigned_user' => $data['assigned']]);
                $leaves->whereIn('leave_requester_userid', [request('for_assigned_user')]);
    ...
    public function countProjects($data = []) {
        ...
        if (isset($data['assigned']) && is_numeric($data['assigned'])) {
         ...
         //Cad status
        if (isset($data['project_custom_field_41']) && $data['project_custom_field_41'] != '' ) {
             $projects->where('project_custom_field_41', $data['project_custom_field_41']);
        }
