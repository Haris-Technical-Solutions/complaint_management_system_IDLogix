...
class RoleRepository {
    ...
    public function create() {
        ...
        $role->role_cads = (in_array(request('role_cads'), $valid)) ? request('role_cads') : 0;
        $role->role_cad_scope = (request('role_cad_scope') == 'on') ? 'global' : 'own';

        $role->role_calander = (in_array(request('role_calander'), $valid)) ? request('role_calander') : 0;
        $role->role_calander_scope = (request('role_calander_scope') == 'on') ? 'global' : 'own';
