...
class CommentRepository {
    ...
    public function search($id = '') {
        ...
        if (request()->filled('commentresource_type')) {
         //   $comments->where('commentresource_type', request('commentresource_type'));
            ...
            if(request('commentresource_type')=='project'){
                     ...
                     $comments->where(function($qery) 
                            {
                               if (request()->filled('commentresource_id')) {
                                          
                                $project = \App\Models\Project::find( request('commentresource_id'));
                               }
                               
                                $qery->where(function($qry) {
                                        $qry->where('commentresource_type', request('commentresource_type'));
                                          if (request()->filled('commentresource_id')) {
                                                $qry->where('commentresource_id', request('commentresource_id'));
                                            }
                                
                                });
                                
                                if( $project)
                                $qery->orWhere(function($qry) use ($project)
                                    {

                                        $qry->where('commentresource_type', 'lead')
                                        ->where('commentresource_id', $project->project_created_from_leadid);
        ...
        //filters: resource type
        if (request()->filled('commentresource_id') && request('commentresource_type')!='project') {
