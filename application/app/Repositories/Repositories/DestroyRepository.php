...
/**
     ...
     * @return bool or id of record
     */
    public function destroyFolder($folder_id) {
        ...
        $filefolder = \App\Models\FileGroup::find($folder_id);
        //get file and validate
        if ($file = \App\Models\File::Where('file_group_id', $folder_id)->first()) {
            Log::error("folder could not be deleted. File exist in it.", ['process' => '[destroy][file]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        ...
        //delete events and events tracking
        if ($events = \App\Models\Event::Where('event_item', 'folder')->Where('event_item_id', $folder_id)->get()) {
            foreach ($events as $event) {
                $event->trackings()->delete();
                $event->delete();
            }
