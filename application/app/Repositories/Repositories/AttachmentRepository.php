...
class AttachmentRepository {
    ...
    public function search($id = '') {
        ...
        if (request()->filled('filegroup_id')) {
            $attachments->where('attachment_folder_id', request('filegroup_id'));
        }
        ...
        //filters: resource type
        if (request()->filled('attachmentresource_ids')) {
            $attachments->whereIn('attachmentresource_id', request('attachmentresource_ids'));
        }
    ...
    public function create($data = '') {
        ...
        $attachment->attachment_folder_id = $data['attachment_folder_id']??0;
